...
describe("kubernetes Pod runner functions", () => {
  ...
  describe("prepareRunPodSpec", () => {
    ...
    it("should include non-blacklisted pod spec fields in the generated pod spec", async () => {
      const podSpec = getResourcePodSpec(helmTarget)
      expect(podSpec).to.eql({
        // This field is blacklisted in `runPodSpecBlacklist`, so it shouldn't appear in the generated pod spec below.
        ...
        // This field is *not* blacklisted in `runPodSpecBlacklist`, so it *should* appear in the generated
        // pod spec below.
        nodeSelector: {
          diskType: "ssd",
        },
      ...
      expect(generatedPodSpec).to.eql({
        nodeSelector: {
          // `nodeSelector` is not blacklisted, so it should be propagated to here.
          diskType: "ssd",
        },
